...
[assembly: Xamarin.Forms.Dependency(typeof(TrackingService_iOS))]

namespace Param_ItemNamespace.iOS.Services
{
    public class TrackingService_iOS : TrackingServiceBase, ITrackingService
    {
        public TrackingService_iOS()
        {
                    ...
                    lock (_lockObject)
                    {
                        foreach (var item in TrackingServiceMetricCollection)
                        {
                            if (item.Value.Measurements.Count > 0 && item.Value.Start.Add(item.Value.MetricTimeSpan) <= DateTime.Now && AppLoggingLevel >= item.Value.MetricLoggingLevel)
                            {
                                ...
                                Device.BeginInvokeOnMainThread(() => MetricsManager.TrackEvent($"Metric:{item.Key}", metricMeasureTuple.Item1, metricMeasureTuple.Item2));
                            }
                        }
                    }
                }
